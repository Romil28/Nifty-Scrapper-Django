...
encryption_key = Fernet(settings.ENCRYPT_KEY)
	...
	try:
		if request and request.method == 'POST':
			...
			if content:
				...
				encrypted_text = encryption_key.encrypt(str(content).encode('ascii'))
		...
		HttpResponse({"message": ex, "Error": "Something went wrong!"})
		raise ex
			...
			if request and request.method == 'POST':
				...
				if url:
					...
					if db_data and db_data[0] and "snippet_data" in db_data[0] and db_data[0]["snippet_data"]:
				        ...
				        decoded_text = encryption_key.decrypt(data).decode("ascii")
				        if decoded_text:
